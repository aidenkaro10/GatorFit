...
(function () {
  ...
  function showBootError(err) {
    ...
    ov.innerHTML = `
      <div class="overlay-inner" style="text-align:center">
        <div class="big-logo logo brand-logo"><img src="https://www.peakpulseclub.com/cdn/shop/files/white_text_peak_pulse_logo_with_sun.png?v=1750709848&width=600" alt="Peak Pulse" onerror="this.parentNode.classList.add('no-img')" /><span class="logo-fallback"><span class="sun">☀</span> PEAK PULSE</span></div>
        <p class="muted">${esc(err.message || "Something went wrong.")}</p>
        <button class="btn btn-primary" data-action="app-retry">Try again</button>
      </div>`;
  }
  ...
  function showAuth() {
    ...
    ov.hidden = false;
    ov.innerHTML = `
      <form class="overlay-inner" id="authForm" novalidate>
        <div class="big-logo logo brand-logo"><img src="https://www.peakpulseclub.com/cdn/shop/files/white_text_peak_pulse_logo_with_sun.png?v=1750709848&width=600" alt="Peak Pulse" onerror="this.parentNode.classList.add('no-img')" /><span class="logo-fallback"><span class="sun">☀</span> PEAK PULSE</span></div>
        <div class="tagline"><b>GatorFit</b> · Where movement meets momentum.</div>
        <input id="authEmail" type="email" placeholder="Email (you@ufl.edu)" autocomplete="email" />
        <input id="authPassword" type="password" placeholder="Password (6+ characters)" autocomplete="current-password" minlength="6" />
        <button class="btn btn-primary" type="submit" id="authBtn">Let's go</button>
  ...
  function showOnboarding() {
    ...
    ov.hidden = false;
    ov.innerHTML = `
      <form class="overlay-inner" id="onboardForm" novalidate>
        <div class="big-logo logo brand-logo"><img src="https://www.peakpulseclub.com/cdn/shop/files/white_text_peak_pulse_logo_with_sun.png?v=1750709848&width=600" alt="Peak Pulse" onerror="this.parentNode.classList.add('no-img')" /><span class="logo-fallback"><span class="sun">☀</span> PEAK PULSE</span></div>
        <div class="tagline">Almost in. What should we call you?</div>
        <input id="obName" placeholder="Your name" maxlength="40" autocomplete="off" />
        <select id="obGreek">
        ...
        <button class="btn btn-primary" type="submit" id="obBtn">Let's go</button>
        <button class="btn" type="button" data-action="app-signout" style="width:100%;margin-top:8px">Use a different email</button>
      </form>`;
  ...
  // The streak line under the points/rank boxes. streak = { current, best, loggedToday } or null.
  function streakLineHTML(streak) {
    if (!streak) return "";
    const n = streak.current;
    let text = "Log anything today to start a streak 🔥";
    if (n >= 2) text = `🔥 ${n}-day streak`;
    else if (n === 1) text = streak.loggedToday ? "🔥 1-day streak. Keep it going tomorrow" : "🔥 1-day streak";
    // Streak is alive from yesterday but nothing logged yet today
    const nudge = n > 0 && !streak.loggedToday ? `<span style="opacity:.7"> · log today to keep it</span>` : "";
    return `<div class="me-sub" id="myStreak" style="white-space:normal;margin-top:10px">${esc(text)}${nudge}</div>`;
  }
    ...
    $("header").innerHTML = `
      <div class="me-card">
        <div style="display:flex;gap:10px;align-items:center">
          ${PP.ui.avatarHTML(me.display_name, { me: true })}
